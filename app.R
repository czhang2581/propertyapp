...
mon<-today()%>% str_extract("-\\d+-")%>%str_remove_all("-") %>%as.numeric()
mont <- mon-1
yr<-today()%>% str_extract("\\d+-")%>%str_remove_all("-")
yrr <- str_sub(yr,-2,-1)

monpool <- c("Jan","Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep" , "Oct" ,"Nov", "Dec")
...
ui <- dashboardPage(
    ...
    dashboardSidebar(
      sidebarMenu(
        ...
        #menuItem("Top Seller", tabName = "fir", icon = icon ("fire", lib = "glyphicon"))
      )
    ),
    ...
    dashboardBody(
      ...
      tabItems(
        ...
        #tabItem(tabName = "fir",
               # fluidRow(
                 # box( title= "Best Selling Development", width = 12, height = "150px", solidHeader = T,
                  #  selectInput("dev6", yr, monpool[1:mon-1], selected = monpool[mon-1])
                 # )
                #),
               #fluidRow(
...
server <- function(input, output, session) {
    ...
    #output$toptable<-function(){
     # prop <- list.load("AllD.RData")
     # prop <- list.filter(prop, length(transaction) >= 20)
     ...
     # put <- input$dev6
      #put <- match(input$dev6,month.abb)
      #if(length(put <2)){
      #  put = str_c(0,put)
     # }
      ...
      #for (i in 1:length(prop)){
       # na[i] = prop[[i]]$project
      #  x = prop[[i]]$transaction
       # x = x[lapply(x, function(x) x$contractDate) == str_c(put, yrr)]
       # nu[i] = x %>% length()
       # avg[i] = lapply(x,"[", "price") %>% unlist()%>%as.numeric() %>% mean() %>%round(0)
        #pr<-lapply(x,"[", "price") %>% unlist()%>%as.numeric()
       # ar<-lapply(x,"[", "area") %>% unlist()%>%as.numeric()*10.76391041671
       # jj<-pr/ar
       # psf[i] = mean(jj)%>%round(0)
      #}

      
      
     # res <- data.frame(Development = na%>%unlist()%>%tolower()%>%str_to_title(), Sold = nu%>%unlist(), 
                       # Price  = str_c('$',
                                      # format(avg%>% unlist()%>%as.numeric()%>%round(0), big.mark = ',', 
                                             # small.interval = 3,digits = 4)), 
                       # PSF = str_c('$',
                                    #format(psf%>%unlist()%>%as.numeric()%>%round(0),big.mark = ',', 
                                          # small.interval = 3,digits = 4))
      #)
      ...
      #res<- res[order(-res$Sold),]
      #res <- res[1:15,]
      #
      #for (i in 1:length(res)){
        #if (res[i, "Development"] == "Parc Clematis"){
          #res[i, "Sold"] = res[i, "Sold"]/2
      #  }
     # }
      
      
      #rownames(res) <- NULL
      #res[order(-res$Sold),] %>% kbl(format = "html", align="llccc", escape=F) %>% kable_material(protect_latex = T)%>% row_spec(0, bold=T, color = 'white', background = "#bf9000") %>% 
        #kable_styling(full_width = T, font_size = 16) %>% row_spec(1:nrow(pric), color = 'black', background = "white")
   ...
   observe({
    ...
    output$price <- renderPlot({
        ...
        g<-ggplot(selbed) +
            ...
            coord_cartesian(xlim= c(min(selbed$Date.of.Sale), max(selbed$Date.of.Sale)), clip = 'off')+ theme(plot.margin = unit(c(1,8,1,1), "lines"),
    ...
    output$comppric <- renderPlot({
       ...
       gg<-ggplot(mydd,aes(x= Size,y=Avg,colour = Development)) +
           ...
           coord_cartesian(xlim= c(min(as.numeric(mydd$Size)), max(as.numeric(mydd$Size))), clip = 'off')+ theme(plot.margin = unit(c(1,8,1,1), "lines"))
       gg
    })
    ...
    output$comppric2 <- renderPlot({
        ...
        ggplot(mydd2,aes(x= Size,y=Avg,colour = Development)) +
            ...
            geom_text(data=mydd2,aes(x=Size,y=Avg,label = format(str_c('$',round(Avg,0)), big.mark = ',', 
                                                                 small.interval = 3)), position = position_dodge(width=0.8), size = 4.5,show.legend = F) + 
            theme(panel.grid.minor.x =element_blank())+
            geom_hline(data = mydd2, aes(yintercept = Avg_Total, colour = Development),linetype = "dashed", show.legend = F) + 
            ...
            theme(legend.position="top") + scale_x_discrete(labels= c("<600 sq ft", "600-800 sq ft","801-1000 sq ft", "1000+ sq ft"))+
            coord_cartesian(xlim= c(min(as.numeric(mydd2$Size)), max(as.numeric(mydd2$Size))+0.25), clip = 'off')+ theme(plot.margin = unit(c(1,8,1,1), "lines"))
    })
